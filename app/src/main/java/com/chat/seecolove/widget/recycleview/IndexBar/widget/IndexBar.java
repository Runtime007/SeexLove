package com.chat.seecolove.widget.recycleview.IndexBar.widget;
...
import com.chat.seecolove.widget.recycleview.IndexBar.helper.IIndexBarDataHelper;
import com.chat.seecolove.widget.recycleview.IndexBar.helper.IndexBarDataHelperImpl;
...
public class IndexBar extends View {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        stockPaint.setColor(ContextCompat.getColor(context, R.color.black_26));
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < mIndexDatas.size(); i++) {
            ...
            //measureHeight = Math.max(indexBounds.height()+DensityUtil.px2dip(mContext,mIndexDatas.size() * 13), measureHeight);//循环结束后，得到index的最大高度，然后*size
            measureHeight = Math.max(indexBounds.height()+ mIndexDatas.size() * 5, measureHeight);
        }
        ...
        switch (hMode) {
            case MeasureSpec.EXACTLY:
                ...
                break;
            case MeasureSpec.AT_MOST:
                //measureHeight = Math.min(measureHeight + DensityUtil.px2dip(mContext,mIndexDatas.size() * 10), hSize + DensityUtil.px2dip(mContext,mIndexDatas.size() * 10));//wSize此时是父控件能给子View分配的最大空间
    ...
    protected void onDraw(Canvas canvas) {
        ...
        r1.top = 0;
        r1.bottom = getHeight();
        canvas.drawRoundRect(r1, 10, 10, stockPaint);
